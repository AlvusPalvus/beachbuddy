...
const ReduxDemo = () => {
    ...
    const inputAddress: string = useAppSelector(
        (state) => state.userOptions.inputAddress
    );
    const origin: LatLngLiteral = useAppSelector(
        (state) => state.userOptions.origin
    );
    const travelMode: TravelMode = useAppSelector(
    ...
    return (
        <div>
            ...
            {origin && (
                <p>
                    User origin: {origin.lat}, {origin.lng}
                </p>
            )}
